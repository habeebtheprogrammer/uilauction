...
class Filter extends Component {
    ...
    filter(){
        ...
        // this.setState({ searching: true })
         window.location.search ===""?
             axios.get(`${apiUrl}/api/filterEvents?orderby=${this.state.orderby}&distance=${this.state.distance}&category=${this.state.category}`).then((res) => {
                 if (res.data.search) {
        ...
        :
        axios.get(`${apiUrl}/api/searchEvents${window.location.search}&orderby=${this.state.orderby}&distance=${this.state.distance}&category=${this.state.category}`).then((res) => {
            if (res.data.search) {
    ...
    render() {
        ...
        return (
            <div>
                <div className="" style={{ position: "relative", padding: "15px 10px 0px", display: "inline-block" }}>
                    <div className="filter-toggle-button " style={{ position: "relative", float: "left" }}>
                        ...
                        <div className="card filter-toggle-div left-align">
                            ...
                            <p onClick={(e) => this.update("viewed")}> Most trending</p>
                            <p onClick={(e) => this.update("newest")}>Newest events</p>
                            <p onClick={(e) => this.update("oldest")}>Older events</p>
                        </div>
                    </div>
                </div>
                {/* <div className="" style={{ position: "relative", padding: "15px 10px 0px", display: "inline-block" }}>
