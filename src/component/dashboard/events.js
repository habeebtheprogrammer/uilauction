...
class Events2 extends Component {
    ...
    componentWillMount() {
        ...
        axios.get(`${apiUrl}/api/eventsById2?id=${this.props.match.params.id}&&token=${token}`).then((res) => {
            if (res.data.event) {
    ...
    submitform(e) {
        ...
        var token = localStorage.getItem("jwToken")
        this.setState({isLoading:true,error:"",success:""})
        axios.post(`${apiUrl}/api/editevent`, {...this.state,token,id:this.state.events._id}).then((res) => {
            if (res.data.success) {
                        ...
                        <div className="container " >
                            ...
                            <div className="row " style={{ margin: "0px" }}>
                                <div className="col s1">
                                    <h5>
                                        <img src={this.state.events.imgUrl} width="60px" height="60px" className="circle" alt="Image" />
                                    </h5>
                                </div>
                                <div className="col s8  " >

                                    <h5 style={{ fontFamily: "avenirBold", padding: "10px 0px 0px" }}>
                                        {this.substr(this.state.events.title, 50)}</h5>
                                    {this.substr(this.state.events.description, 70)}
                            ...
                            <form onSubmit={this.submitform}>
                                ...
                                <Row>
                                    ...
                                    <Col s={12}>
                                        ...
                                        <label>Title  </label>
                                        <input name="title" placeholder={this.state.events.title} onChange={this.typing.bind(this)} type="text" className="input" title="" />
                                    </Col>
                                    <Col s={6}>

                                        <label>Location   </label>
                                        <input name="location" placeholder={this.state.events.location} onChange={this.typing.bind(this)} type="text" className="input" title="" />
                                    </Col>

                                    <Col s={6}>

                                        <label>Link to Ticket  </label>
                                        <input name="ticket" placeholder={this.state.events.ticket} onChange={this.typing.bind(this)} type="text" className="input" title="" />
                                    </Col>

                                    <Input s={6} type='select' name="startTime" defaultValue='0' label="From" onChange={this.typing.bind(this)}>
                                        <option className="grey-text text-darken-2" >closed</option>
                                        {time.map((time, key) => (
                                            <option key={key} className="grey-text text-darken-2" value={time}>{time}</option>
                                        ))}
                                    </Input>
                                    <Input s={6} type='select' name="stopTime" defaultValue='0' label="To" onChange={this.typing.bind(this)}>
                                        <option className="grey-text text-darken-2" >closed</option>
                                        {time.map((time, key) => (
                                    ...
                                    <Col s={12}>
                                        ...
                                        <label>Date </label>
                                        <input name="checkedDate" placeholder={this.state.events.checkedDate} onChange={this.typing.bind(this)} type="date" className="input" title="" />
                                    </Col>
                                    <Col s={12}>

                                        <label>Description </label>
                                        <textarea placeholder={this.state.events.description} onChange={this.typing.bind(this)} name="description" cols="30" rows="10"></textarea>
                                    </Col>
                                </Row>
                                ...
                                <a className="btn red z-depth-0 lighten-1 right" onClick={this.deleteevent}>
